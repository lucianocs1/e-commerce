...
<div className="exibicao-produto">
  <div className="exibicao-produto-esquerda">
    ...
    <div className="exibicao-produto-imagem">
      <img className="exibicao-produto-imagem-principal" src={backend_url + product.image} alt="img" />
    </div>
  </div>
  <div className="exibicao-produto-direita">
    <h1>{product.name}</h1>
    <div className="exibicao-produto-direita-precos">
      <div className="exibicao-produto-preco-antigo">{currency}{product.old_price}</div>
      <div className="exibicao-produto-preco-novo">{currency}{product.new_price}</div>
    </div>
      ...
      <div className="exibicao-produto-direita-tamanho">
        ...
        <div className="exibicao-produto-direita-tamanhos">
          {sizeOptions.map((size) => (
            <div
              key={size}
              className={`opcao-tamanho ${selectedSize === size ? 'selecionado' : ''}`}
              onClick={() => handleSizeClick(size)}
            >
              {size}
            </div>
    ...
    <button onClick={() => addToCart(product.id)}>Adicionar ao carrinho</button>
    <p className="exibicao-produto-direita-categoria"><span>Categoria :</span> {product.category}</p>
    <p className="exibicao-produto-direita-tags"><span>Tags :</span> {product.tags}</p>
  </div>
</div>
